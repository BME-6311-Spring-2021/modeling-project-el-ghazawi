...
to Run-Simulation
  ...
  proliferate-microbes
  proliferate-neutrophils
  proliferate-macrophages
  migrate
  Leukocyte-Actions

  if (any? bacteria) and (not any? microbes) [
   ...
   if (any? bacteria) and (any? microbes) [
    if Neutrophil_Response = "Low" and ticks mod 3 = 0 [create-neutrophils ceiling (.05 * Immune-Cell-Infiltration) [setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]]
    if Neutrophil_Response = "Normal" [create-neutrophils ceiling (0.5 * Immune-Cell-Infiltration)[setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]]
    if Neutrophil_Response = "High" [create-neutrophils ceiling (1 * Immune-Cell-Infiltration) [setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]]
  ...
  set number-of-cells count microbes
  set clock clock + 1
  tick
...
to infect
  ask patch 0 0 [
    ask n-of Initial-Bacteria patches in-radius 15[
     ...
     ask n-of (Initial-Microbes / 2 ) patches in-radius 15 [sprout 1 [set breed microbes set shape "bug" set age random prolif-rate]]
...
to proliferate-microbes
  ask microbes [
    if age mod prolif-rate = 0 and count microbes-on neighbors < 8                ; cell division happens for every cell at the same time (i.e. clock tick) according to the 'prolif-rate' set on the interface tab. See 'proliferate' sub-routine below.
      [
        hatch 1 [
          let attempted-moves  0
          while [any? other microbes-here and attempted-moves < 16]
          [
            if not any? neighbors with [wound = 1 and not any? microbes-here][die]
            move-to one-of neighbors with [wound = 1 and not any? microbes-here]
            set attempted-moves attempted-moves + 1]
...
to migrate
  ...
  ask microbes
 [ if any? neighbors with [not any? microbes-here and wound = 1]
    [move-to one-of neighbors with [not any? microbes-here]]]

  ask neutrophils ;neutrophils move towards bacteria in their homine range, or to a random neighboring patch
  [(
    ifelse (any? bacteria in-radius Neutrophil-Homing-Range) and (not any? microbes in-radius Neutrophil-Homing-Range) [
      face min-one-of bacteria [distance myself]
      fd 1
    ]
    (any? bacteria in-radius Neutrophil-Homing-Range) and (any? microbes in-radius Neutrophil-Homing-Range) [
      face min-one-of bacteria [distance myself]
      face min-one-of microbes [distance myself ]
  ...
  [(
    ...
    any? microbes-on neighbors
    [move-to one-of neighbors with [any? microbes-here]]
    [move-to one-of neighbors] )
  ]
end

to Leukocyte-Actions
  ask neutrophils [(ifelse
    any? bacteria-on patch-here and not any? microbes-on patch-here
    [
      ask patch-here [set debris debris + (count bacteria-here)]
      ask bacteria-on patch-here [die]

    ]
    any? bacteria-on patch-here and any? microbes-on patch-here
    [ask patch-here [set debris debris + (count bacteria-here) + (count microbes-here)]
      ask bacteria-on patch-here [die]
      ask microbes-on patch-here[die]
    ]
    not any? bacteria-on patch-here and any? microbes-on patch-here
    [ask patch-here [set debris debris + (count microbes-here)]
      ask microbes-on patch-here [die] ]

 ) ]


 ask macrophages [( ifelse
    any? bacteria-on patch-here and not any? microbes-on patch-here
    [ask bacteria-on patch-here [die]]
    any? bacteria-on patch-here and any? microbes-on patch-here
    [ask bacteria-on patch-here [die] ask microbes-on patch-here [die]]
    not any? bacteria-on patch-here and any? microbes-on patch-here
    [ask microbes-on patch-here [die]]
    )
...
"Biofilm" 1.0 0 -15973838 true "" "plot count microbes + count bacteria"
...
@#$#@#$#@
<experiments>
  <experiment name="Effect of Biofilm" repetitions="3" runMetricsEveryStep="true">
    <setup>setup
infect</setup>
    ...
    <metric>count microbes</metric>
    <metric>count microbes + count bacteria</metric>
    ...
    <enumeratedValueSet variable="Initial-microbes">
      <value value="0"/>
      <value value="5"/>
      <value value="10"/>
    </enumeratedValueSet>
